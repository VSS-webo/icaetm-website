...
// export default function About() {
//   return (
//     <section id="about" className="py-24 bg-white-950">   {/*changed bg-slate to bg-white*/}
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-28">
        ...
        >
          ...
          <div>
            ...
            <button
              onClick={() => setShowPVG(!showPVG)}
              className="mt-4 text-indigo-600 font-medium text-sm hover:underline transition"
            >
              {showPVG ? 'Read less ↑' : 'Read more ↓'}
            </button>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            ...
            <motion.div
              initial={false}
              animate={{
                height: showCOETM ? 'auto' : 0,
                opacity: showCOETM ? 1 : 0,
              }}
              transition={{ duration: 0.4, ease: 'easeInOut' }}
              className="overflow-hidden"
            >
            ...
            <button
              onClick={() => setShowCOETM(!showCOETM)}
              className="mt-4 text-indigo-600 font-medium text-sm hover:underline transition"
            >
              {showCOETM ? 'Read less ↑' : 'Read more ↓'}
            </button>
          </div>
        </motion.div>
        ...
        >
          <h3 className="text-3xl font-semibold text-indigo-700 text-center mb-5">
            About Pune
          </h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {punePlaces.map((place, idx) => (
              <div
                key={idx}
                className="relative h-60 rounded-3xl overflow-hidden shadow-lg group"
              >
                <Image
                  src={place.src}
                  alt={place.name}
